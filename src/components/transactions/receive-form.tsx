...
export function ReceiveForm() {
  ...
  async function handleSubmit() {
    if (!canCreate) {
      setErrors({ form: "You do not have permission to record receipts." });
      return;
    }
    ...
    try {
      ...
      // Success — REC number came from the DB response, never built here.
      setSaved(created);
      queryClient.invalidateQueries({ queryKey: ["transactions"] });

      // reset form for "Receive Another"
      setMaterialId(null);
      setCompanyId(null);
      setPieces("");
      setAmount("");
  ...
  // ----- success state -----
  if (saved) {
    const directionLabel = saved.type === "received" ? "Received" : "Given";
    return (
      <div className="mx-auto max-w-md rounded-xl border bg-white p-6 text-center shadow-sm">
        ...
        <p className="text-sm font-medium text-emerald-700">{directionLabel}</p>
  ...
  return (
    <div className="mx-auto max-w-md">
      ...
      <div className="grid gap-4">
        ...
        {/* From company */}
        <Field label="From" error={errors.company}>
          <EntityCombobox
